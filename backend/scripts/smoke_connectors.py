...
class ConnectorSmokeRunner:
    ...
    def _run_provider(
        ...
        *,
        ...
        oauth_status = (
            integration.get("oauth_status") if isinstance(integration, dict) else None
        )
        ...
        if final_snapshot.get("circuit_open_until"):
            circuit_open_until = self._parse_dt(
                str(final_snapshot["circuit_open_until"])
            )
    ...
    def _select_or_create_connector(
        ...
        *,
        ...
        if not isinstance(connector, dict):
            raise ConnectorSmokeError(
                f"Unexpected connector payload for provider {provider}"
            )
        return connector
    ...
    def _fetch_connectors(self) -> list[dict[str, Any]]:
        response = self.client.get(
            "/api/v1/integrations/connectors", headers=self.headers
        )
    ...
    def _wait_for_connector_state(
        ...
        *,
        ...
        LOGGER.warning(
            "Timed out waiting for %s on connector_id=%s", state_label, connector_id
        )
        return False
    ...
    def _sync_completed_predicate(
        ...
        if before_sync_value and str(before_sync_value) == str(
    ...
    def _raise_for_status(response: httpx.Response, action: str) -> None:
        ...
        raise ConnectorSmokeError(
            f"Failed to {action}: {response.status_code} {message}"
        )


def _parse_args(argv: list[str]) -> ConnectorSmokeOptions:
    parser = argparse.ArgumentParser(
        description="Run a connector smoke test across providers."
    )
    parser.add_argument(
        "--base-url",
    ...
    parser.add_argument(
        "--access-token", default=os.getenv("AKS_SMOKE_ACCESS_TOKEN") or ""
    )
    parser.add_argument(
        "--providers",
    ...
    providers = tuple(
        provider.strip()
        for provider in str(args.providers).split(",")
        if provider.strip()
    )
    if not providers:
    ...
    if not args.access_token and not args.dry_run:
        raise SystemExit(
            "Missing access token. Set AKS_SMOKE_ACCESS_TOKEN or pass --access-token."
        )
