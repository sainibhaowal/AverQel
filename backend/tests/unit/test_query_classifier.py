...
def test_query_classifier_comparison():
    assert QueryClassifier.classify("Compare the Q3 revenue vs Q4 revenue") == QueryType.COMPARISON
    assert (
        QueryClassifier.classify("What is the difference between A and B?") == QueryType.COMPARISON
    )


def test_query_classifier_summarization():
    assert (
        QueryClassifier.classify("Can you summarize the second chapter?") == QueryType.SUMMARIZATION
    )
    assert (
...
def test_query_classifier_verification():
    assert QueryClassifier.classify("Is it true that the CEO resigned?") == QueryType.VERIFICATION
    assert (
...
def test_query_classifier_exploratory():
    ...
    assert QueryClassifier.classify("Explain how the new algorithm works.") == QueryType.EXPLORATORY


def test_query_classifier_factual():
    assert QueryClassifier.classify("What is the capital of France?") == QueryType.FACTUAL
    assert QueryClassifier.classify("When was this document created?") == QueryType.FACTUAL
