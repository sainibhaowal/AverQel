...
class MCPServer(Base):
    ...
    account_identity: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, server_default=text("'{}'::jsonb")
    )
    ...
    status: Mapped[str] = mapped_column(
        String(32), nullable=False, server_default=text("'disconnected'"), index=True
    )
...
class MCPRegistryEntry(Base):
    ...
    oauth_requirements: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, server_default=text("'{}'::jsonb")
    )
    oauth_profile: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, server_default=text("'{}'::jsonb")
    )
    requested_scopes: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, server_default=text("'[]'::jsonb")
    )
    supported_products: Mapped[list[str]] = mapped_column(
        JSONB, nullable=False, server_default=text("'[]'::jsonb")
    )
    ...
    catalog_badges: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, server_default=text("'{}'::jsonb")
    )
    ...
    tool_risk_summary: Mapped[dict[str, Any]] = mapped_column(
        JSONB, nullable=False, server_default=text("'{}'::jsonb")
    )
    ...
    trust_status: Mapped[str] = mapped_column(
        String(24), nullable=False, server_default=text("'discovered'"), index=True
    )
    ...
    health_status: Mapped[str] = mapped_column(
        String(32), nullable=False, server_default=text("'not_checked'"), index=True
    )
...
class MCPEvent(Base):
    ...
    __table_args__ = (
        UniqueConstraint("server_id", "sequence", name="uq_mcp_events_server_sequence"),
    )
