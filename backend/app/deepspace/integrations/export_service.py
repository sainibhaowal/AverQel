...
class DeepSpaceExportService:
    ...
    # --------------------------------------------------------------- PPTX
    def generate_pptx(self, html_content: str, title: str = "DeepSpace Note") -> io.BytesIO:
        """Create a bounded, editable presentation from note headings/content."""
        ...
        if subtitle is not None:
            subtitle.text = "Generated by AverQel DeepSpace"
        soup = BeautifulSoup(html_content, "html.parser")
        blocks = [
            item.get_text(" ", strip=True)
            for item in soup.find_all(["h1", "h2", "h3", "p", "li"])
            if isinstance(item, Tag) and item.get_text(" ", strip=True)
        ]
        for offset in range(0, min(len(blocks), 80), 6):
            slide = presentation.slides.add_slide(presentation.slide_layouts[1])
            slide.shapes.title.text = f"{title} · {offset // 6 + 1}"
            frame = slide.placeholders[1].text_frame
            frame.clear()
            for index, value in enumerate(blocks[offset : offset + 6]):
                paragraph = frame.paragraphs[0] if index == 0 else frame.add_paragraph()
                paragraph.text = value[:1200]
                paragraph.font.size = Pt(18)
            # Keep the generated deck inside the normal slide canvas.
