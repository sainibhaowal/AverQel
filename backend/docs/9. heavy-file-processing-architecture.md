...
Still deliberately separate from this change: multi-dataset joins, user-defined
SQL notebooks, materialized chart artifacts, and a browser virtual-scrolling
grid. They require separate tenant-scoped product contracts and authenticated
browser E2E coverage; they must not be silently introduced through a generic
query endpoint.
