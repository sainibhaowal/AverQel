...
export function saveMCPActiveContext(context: Omit<MCPActiveContext, "updated_at">): void {
  ...
  window.localStorage.setItem(
    MCP_ACTIVE_CONTEXT_KEY,
    JSON.stringify({ ...context, updated_at: Date.now() }),
  );
}
...
export function readMCPActiveContext(): MCPActiveContext | null {
  ...
  try {
    const value = JSON.parse(
      window.localStorage.getItem(MCP_ACTIVE_CONTEXT_KEY) || "null",
    ) as Partial<MCPActiveContext> | null;
    if (
      !value ||
      typeof value.updated_at !== "number" ||
      Date.now() - value.updated_at > MAX_CONTEXT_AGE_MS
    )
      return null;
    return {
