...
const { providerEntry } = vi.hoisted(() => ({
  providerEntry: {
    id: "provider-1",
    name: "Route Provider",
    publisher: "Provider",
    ...
    tool_preview: [
      { name: "read_data", description: "Read data", category: "Data", risk_labels: ["read"] },
    ],
    tools: [
      { name: "read_data", description: "Read data", category: "Data", risk_labels: ["read"] },
    ],
...
describe("MCP provider detail route", () => {
  it("loads the API-backed provider detail page", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("heading", { name: "Route Provider" })).toBeInTheDocument(),
    );
