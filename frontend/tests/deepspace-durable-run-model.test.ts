...
describe("durable run model", () => {
  it("keeps the newest event cursor and never regresses the checkpoint", () => {
    const started = deepSpaceThreadReducer(initialDeepSpaceThreadState, {
      ...
      event: {
        event: "meta",
        data: { durable_run_id: "run-1", sequence: 4, checkpoint_sequence: 3, status: "running" },
      },
    });
    const advanced = deepSpaceThreadReducer(started, {
      ...
      event: {
        event: "meta",
        data: { durable_run_id: "run-1", sequence: 8, checkpoint_sequence: 2, status: "running" },
      },
    });
    expect(advanced.durableRun).toMatchObject({
      runId: "run-1",
      lastSequence: 8,
