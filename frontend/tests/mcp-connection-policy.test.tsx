...
vi.mock("@/lib/mcp-api", async () => ({ ...(await vi.importActual("@/lib/mcp-api")), updateMCPPolicy }));

describe("MCPConnectionPolicyPanel", () => {
  beforeEach(() => { updateMCPPolicy.mockReset().mockResolvedValue(policy); });
  ...
  it("preserves conservative read-only and approval settings when saved", async () => {
    ...
    await waitFor(() => expect(updateMCPPolicy).toHaveBeenCalledWith("server-1", expect.objectContaining({
      read_only: true,
      risk_ceiling: "read",
