...
export default function MCPDashboard() {
  ...
  return (
    <main className="w-full min-w-0 space-y-8 pb-10">
        ...
        <DashboardSectionHeader
          ...
          actions={
            <div className="flex gap-2">
              <button type="button" className={`rounded px-3 py-2 text-sm ${tab === "marketplace" ? "bg-white/15 text-white" : "border border-white/10 text-slate-300"}`} onClick={() => setTab("marketplace")}>Marketplace</button>
              <button type="button" className={`rounded px-3 py-2 text-sm ${tab === "installed" ? "bg-white/15 text-white" : "border border-white/10 text-slate-300"}`} onClick={() => setTab("installed")}>Installed ({installed.length})</button>
            </div>
          }
        />
        {tab === "marketplace" && <div className="space-y-3"><div className="flex flex-wrap gap-3"><label className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-white/10 bg-black/10 px-3 py-2"><Search className="h-3.5 w-3.5 text-slate-500" aria-hidden="true" /><input className="w-full bg-transparent text-sm text-slate-200 outline-none placeholder:text-slate-500" placeholder="Search providers and tools" value={q} onChange={(event) => { setPage(1); setQ(event.target.value); }} /></label><MCPFilterSelect label="All transports" value={transport} options={[{ value: "", label: "All transports" }, ...facets.transports.map((item) => ({ value: item, label: transportLabel(item) }))]} onChange={(value) => { setPage(1); setTransport(value); }} /><MCPFilterSelect label="Filter by" value={authType ? `auth:${authType}` : trustStatus ? `trust:${trustStatus}` : ""} options={[{ value: "", label: "Filter by" }, ...facets.auth_types.map((item) => ({ value: `auth:${item}`, label: item })), ...facets.trust_statuses.map((item) => ({ value: `trust:${item}`, label: item }))]} onChange={(value) => { setPage(1); setAuthType(value.startsWith("auth:") ? value.slice(5) : ""); setTrustStatus(value.startsWith("trust:") ? value.slice(6) : ""); }} /><MCPFilterSelect label="Sort by" value={sort} options={[{ value: "default", label: "Sort by" }, { value: "popular", label: "Popular" }, { value: "trending", label: "Trending" }, { value: "new", label: "New" }, { value: "alphabetical", label: "Alphabetical" }]} onChange={(value) => { setPage(1); setSort(value as NonNullable<MarketplaceQuery["sort"]>); }} /><Chip label="Verified" active={verified === true} onClick={() => { setPage(1); setVerified(verified === true ? null : true); }} /></div><button type="button" className="flex w-full items-center gap-2 rounded border border-white/10 px-3 py-2 text-left text-xs text-slate-400 hover:bg-white/[0.03]" onClick={() => setFiltersOpen((open) => !open)}><span>{filtersOpen ? "▾" : "▸"}</span>More filters</button>{filtersOpen && <div className="space-y-3 rounded border border-white/10 p-3"><div className="flex flex-wrap gap-2"><Chip label="All" active={!category} onClick={() => { setPage(1); setCategory(""); }} />{categories.map((item) => <Chip key={item} label={item} active={category === item} onClick={() => { setPage(1); setCategory(item); }} />)}</div><div className="flex flex-wrap gap-2"><Chip label="Official" active={official === true} onClick={() => { setPage(1); setOfficial(official === true ? null : true); }} /><button type="button" className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-slate-400" onClick={clearFilters}>Clear filters</button></div></div>}</div>}
      </header>
      ...
      {tab === "marketplace" ? <section className="space-y-6"><p className="text-sm text-slate-400">{total.toLocaleString()} approved connectors</p>{loading ? <p className="text-sm text-white/60">Loading marketplace…</p> : items.length === 0 ? <p className="rounded-2xl border border-white/10 p-6 text-sm text-white/55">No approved connectors match these filters.</p> : <div className="grid items-stretch gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{items.map((entry) => { const connectedServer = installed.find((server) => server.registry_entry_id === entry.id || (server.provider_slug && server.provider_slug === entry.provider_slug)); return <MCPMarketplaceCard key={entry.id} entry={entry} connectedServer={connectedServer} onConnect={(value) => void connect(value)} onReconnect={(server) => { void startMCPServerOAuth(server.id).then((result) => window.location.assign(result.authorization_url)).catch((reason) => setError(reason instanceof Error ? reason.message : "Unable to reconnect provider.")); }} />; })}</div>}<Pagination page={page} hasNext={page < pages} onPrevious={() => setPage(Math.max(1, page - 1))} onNext={() => setPage(page + 1)} /></section> : <section className="grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">{installed.length === 0 ? <p className="rounded-2xl border border-white/10 p-6 text-sm text-white/55">No MCP connections yet. Connect one from the marketplace.</p> : installed.map((server) => <InstalledCard key={server.id} server={server} clientReady={clientReady} onRefresh={async () => { await refreshMCPServer(server.id); await load(); }} onDisconnect={async () => { if (window.confirm("Disconnect this MCP connection?")) { await deleteMCPServer(server.id); await load(); } }} />)}</section>}
    </main>
  );
}
...
function InstalledCard({ server, clientReady, onRefresh, onDisconnect }: { server: MCPConnection; clientReady: boolean; onRefresh: () => Promise<void>; onDisconnect: () => Promise<void> }) {
