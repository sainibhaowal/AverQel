  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        <label className="flex items-start gap-3 rounded-xl border border-white/10 p-3"><input type="checkbox" checked={draft.default_enabled} onChange={(event) => setDraft({ ...draft, default_enabled: event.target.checked })} className="mt-1" /><span><span className="block text-sm text-white">Enable this connection</span><span className="mt-1 block text-xs text-white/45">Disabled connections are never offered to DeepSpace.</span></span></label>
      ...
      <p className="mt-5 rounded-xl border border-white/10 bg-black/15 p-3 text-xs leading-5 text-white/50">A blocked or disabled tool is removed from DeepSpace&apos;s available tools and checked again immediately before any remote call. Conversation and DeepSpace overrides are configured below.</p>
    </section>
  );
}
