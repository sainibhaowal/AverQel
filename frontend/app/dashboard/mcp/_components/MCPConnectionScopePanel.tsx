    ...
    <section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.02] p-5">
      ...
      <details className="mt-4 group border-t border-white/10 pt-3">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          ...
          <ScopeControl label="DeepSpace mission override" value={deepSpaceId} onChange={setDeepSpaceId} scope="deepspace" current={deepSpace} loading={loading === "deepspace"} onLoad={() => void loadScope("deepspace", deepSpaceId)} onToggle={(enabled) => void toggle("deepspace", deepSpaceId, enabled)} />
        </div>
      </details>
    </section>
  );
  ...
  return (
    <div className="rounded-xl border border-white/10 p-4">
      <label className="block text-sm text-white">
        {label}
        <input value={value} onChange={(event) => onChange(event.target.value)} placeholder={`Enter ${scope} ID`} className="mt-2 w-full rounded-lg border border-white/10 bg-[#111512] px-3 py-2 font-mono text-xs text-white placeholder:text-white/25" />
      </label>
      <div className="mt-3 flex items-center justify-between gap-3">
        <button type="button" onClick={onLoad} disabled={loading || !value.trim()} className="rounded-lg border border-white/10 px-3 py-2 text-xs text-white/75 hover:bg-white/10 disabled:opacity-40">
          {loading ? "Loading…" : "Load scope"}
        </button>
        {current && (
          <label className="flex items-center gap-2 text-xs text-white/65">
            <input type="checkbox" checked={current.enabled} disabled={loading} onChange={(event) => onToggle(event.target.checked)} />
