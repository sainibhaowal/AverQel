...
export default function MCPConnectionScopePanel({ serverId, initialConversationId = "", initialDeepSpaceId = "" }: { serverId: string; initialConversationId?: string; initialDeepSpaceId?: string }) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><div><h2 className="text-lg font-semibold text-white">DeepSpace and conversation access</h2><p className="mt-1 text-sm text-white/55">AverQel automatically uses the active DeepSpace conversation when available. Connections remain disabled for a scope until you explicitly enable it, and ownership is verified server-side.</p></div>{message && <p className="mt-3 text-sm text-amber-200" role="status">{message}</p>}<div className="mt-5 grid gap-4 lg:grid-cols-2"><ScopeControl label="Current conversation" value={conversationId} onChange={setConversationId} scope="conversation" current={conversation} loading={loading === "conversation"} onLoad={() => void loadScope("conversation", conversationId)} onToggle={(enabled) => void toggle("conversation", conversationId, enabled)} /><ScopeControl label="Current DeepSpace mission" value={deepSpaceId} onChange={setDeepSpaceId} scope="deepspace" current={deepSpace} loading={loading === "deepspace"} onLoad={() => void loadScope("deepspace", deepSpaceId)} onToggle={(enabled) => void toggle("deepspace", deepSpaceId, enabled)} /></div><p className="mt-4 text-xs leading-5 text-white/40">A stale or unavailable context stays disabled. Backend ownership checks reject invalid or cross-tenant IDs.</p></section>
  );
}
