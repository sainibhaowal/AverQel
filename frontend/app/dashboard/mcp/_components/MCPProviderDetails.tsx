    ...
    <main className="mx-auto max-w-6xl space-y-8 px-6 py-8">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
        <div className="space-y-6">
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5"><div className="flex items-center justify-between gap-4"><h2 className="text-lg font-semibold text-white">Tools</h2><span className="text-xs text-white/45">{entry.tool_count || entry.tool_preview.length} cataloged</span></div>{entry.tool_preview.length === 0 ? <p className="mt-4 text-sm text-white/50">Tools will be revealed after the connection is authenticated and refreshed.</p> : <div className="mt-4 divide-y divide-white/5">{entry.tool_preview.map((tool) => <div key={tool.name} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-mono text-sm text-emerald-200">{tool.name}</p><p className="mt-1 text-sm text-white/60">{tool.description || "No description provided."}</p></div><div className="flex shrink-0 flex-wrap gap-1.5">{(tool.risk_labels.length ? tool.risk_labels : ["read"]).map((risk) => <span key={risk} className="rounded-full border border-white/10 px-2 py-1 text-[11px] text-white/60">{riskLabel(risk)}</span>)}{tool.category && <span className="rounded-full border border-white/10 px-2 py-1 text-[11px] text-white/45">{tool.category}</span>}</div></div>)}</div>}</section>
        </div>
