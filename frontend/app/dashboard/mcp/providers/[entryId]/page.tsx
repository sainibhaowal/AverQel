...
export default function MCPProviderPage({ params }: { params?: { entryId: string } }) {
  ...
  const connect = async (value: MCPMarketplaceEntry) => {
    ...
    try {
      ...
      else window.location.assign(`/dashboard/mcp/inspector/${encodeURIComponent(result.server.id)}`);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to connect provider."); setConnecting(false); }
  };

  const reconnect = async (server: MCPConnection) => {
    setConnecting(true); setError(null);
    try { const result = await startMCPServerOAuth(server.id); window.location.assign(result.authorization_url); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to reconnect provider."); setConnecting(false); }
  };

  if (error) return <main className="mx-auto max-w-4xl p-6"><p className="rounded border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-200" role="alert">{error}</p></main>;
  if (!entry) return <main className="mx-auto max-w-4xl p-6 text-sm text-white/60">Loading provider details…</main>;
  return <MCPProviderDetails entry={entry} connectedServer={connectedServer} onConnect={(value) => void connect(value)} onReconnect={(server) => void reconnect(server)} connecting={connecting} />;
}
