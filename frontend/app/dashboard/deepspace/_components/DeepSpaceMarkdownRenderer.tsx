...
function ChartPreview({ payload }: { payload: Record<string, unknown> }) {
  const title = typeof payload.title === "string" && payload.title.trim() ? payload.title : "Chart Data";
  const chartType = typeof payload.chart_type === "string" ? payload.chart_type : "bar";
  const series = Array.isArray(payload.series)
    ? payload.series.filter((item): item is Record<string, unknown> => Boolean(item) && typeof item === "object")
    : [];
  return (
    <section role="img" aria-label={title} className="my-4 rounded-xl border border-white/10 bg-black/20 p-4">
      <div className="flex items-center justify-between gap-3">
        <h4 className="font-semibold">{title}</h4>
        <span className="text-xs uppercase text-cyan-300">{chartType}</span>
      </div>
      <div className="mt-2 text-xs text-foreground/60">{series.length} points · JSON chart</div>
      <div className="mt-3 grid gap-1 text-xs text-foreground/75">
        {series.map((point, index) => (
          <div key={`${String(point.label ?? index)}-${index}`} className="flex justify-between gap-4 rounded bg-white/[0.03] px-2 py-1">
...
export default function DeepSpaceMarkdownRenderer({ content, streaming = false }: { content: string; streaming?: boolean }) {
  const components = useMemo<Components>(() => ({
    ...
    code: ({ children, className, ...props }) => {
      ...
      if (!streaming && language === "mermaid" && value.trim()) return <MermaidPreview source={value} />;
      ...
      return (
        <pre className="my-4 overflow-x-auto rounded-xl border border-white/10 bg-black/30 p-4 text-xs leading-6 text-cyan-100">
          <code className={className} {...props}>{value || (streaming ? " " : "")}</code>
        </pre>
      );
    },
    table: ({ children }) => <div className="my-5 overflow-x-auto rounded-xl border border-white/10"><table className="w-full border-collapse text-left">{children}</table></div>,
    th: ({ children }) => <th className="border-b border-white/10 bg-white/5 px-4 py-3 text-xs uppercase">{children}</th>,
    td: ({ children }) => <td className="border-b border-white/5 px-4 py-3 text-sm text-foreground/80">{children}</td>,
    ...
    blockquote: ({ children }) => <blockquote className="my-3 border-l-2 border-cyan-400/50 bg-cyan-400/5 px-4 py-3">{children}</blockquote>,
  }), [streaming]);
  const normalizedContent = useMemo(() => normalizeMarkdown(content), [content]);
  return <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]} components={components}>{normalizedContent}</ReactMarkdown>;
}
